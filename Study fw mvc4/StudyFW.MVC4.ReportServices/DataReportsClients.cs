...
namespace StudyFW.MVC4.ReportServices
{
    public class DataReportsClients : IDataReports
    {
		...
		// HOAVV PHASE 2 EXPORT CSV
        public MemoryStream ExportCSV(DataTable dt1, DataTable dt2)
        {
            MemoryStream stream = new MemoryStream();
            MemoryStream streamKeiCar0 = new MemoryStream();
            MemoryStream streamKeiCar1 = new MemoryStream();
            if (this.listStreamCSV == null)
            {
                this.listStreamCSV = new List<MemoryStream>();
            }
            streamKeiCar0 = OutPutCSVGHQ.ExportCSV(dt1, "\"車輌番号\",\"登録年\",\"登録月\",\"登録日\",\"車台番号\",\"備考欄\"");
            streamKeiCar1 = OutPutCSVGHQ.ExportCSV(dt2, "\"自動車登録番号又は車両番号\",\"登録年\",\"登録月\",\"登録日\",\"車台番号\",\"乗車定員\",\"総排気量又は定格出力\",\"備考欄\"");
            this.listStreamCSV.Add(streamKeiCar0);
            this.listStreamCSV.Add(streamKeiCar1);
            string foderName= DateTime.Now.ToString("yyyyMMdd") + "_meihen.zip";
            string fileName1 = "f "+DateTime.Now.ToString("yyMMdd");
            string fileName2 = "k " + DateTime.Now.ToString("yyMMdd");
            stream = ExportCSVZip(new string[] { fileName2, fileName1 }, foderName);
            return stream;
        }
        ...
        public MemoryStream ExportCSVZip(string[] nameCSV, string nameFileZip)
        {
            ...
            this.zip = new ZipFile();
            //Denpyo_yyyyMMddHHmm
            string foderName = DateTime.Now.ToString("yyyyMMdd") + "_meihen";
            string nameFileDate = foderName;
            // create new stream
            MemoryStream stream = new MemoryStream();
            int i = 0;
            if (this.listStreamCSV != null)
        ...
        public DataTable ExportPDFRD0010(DataTable table)
        {
            DataTable dt = new DataTable();
            DataTable dataSetResult = new DataTable();

            using (DBManager dbManager = new DBManager(ConstantsStpReports.RD0010))
            {
                dbManager.Add("@list", SqlDbType.Structured, table);
                dataSetResult = dbManager.GetDataTable();
            }
        ...
        public DataTable ExportPDFRD0030(DataTable table)
        {
            DataTable dt = new DataTable();
            DataTable dataSetResult = new DataTable();

            using (DBManager dbManager = new DBManager(ConstantsStpReports.RD0030))
            {
                dbManager.Add("@list", SqlDbType.Structured, table);
                dataSetResult = dbManager.GetDataTable();
            }
        ...
        public static class OutPutCSVGHQ
        {
            public static MemoryStream ExportCSV(DataTable dt, string header = "")
            {
                // Create the CSV file to which grid data will be exported.
