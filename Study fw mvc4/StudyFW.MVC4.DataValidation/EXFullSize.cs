...
namespace StudyFW.MVC4.DataValidation
{
	...
	/// Full size check
	/// </summary>
	public class EXFullSize : ValidationAttribute, IClientValidatable
	{
		/// <summary>
		/// The _MSG cd
		/// </summary>
		...
		public EXFullSize( string msgCd, string msgReplaceStr = "" )
		{
			this._msgCd = msgCd;
		...
		public EXFullSize( string msgCd, Type msgReplaceStrResourceType, string msgReplaceStrResourceName )
			: base()
		{
		...
		public IEnumerable<ModelClientValidationRule> GetClientValidationRules( ModelMetadata metadata, ControllerContext context )
		{
			...
			rule.ValidationType = "exfullsize";
			yield return rule;
		}
		...
		protected override ValidationResult IsValid( object value, ValidationContext validationContext )
		{
			...
			if (Encoding.GetEncoding( "shift-jis" ).GetBytes( Convert.ToString( value ) ).Length < Convert.ToString( value ).Length * 2)
			{
				new ValidUtility().SaveError( _PropertyName, ValidUtility.Type.EXFullSize, FormatErrorMessage( validationContext.DisplayName ) );
